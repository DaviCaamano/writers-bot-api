...
import { fetchWorld } from '@/utils/legacy';

const mockPool = pool as jest.Mocked<typeof pool>;
const mockFetchWorldById = fetchWorld as jest.MockedFunction<typeof fetchWorld>;
...
describe('upsertWorld', () => {
  ...
  describe('creating a new world', () => {
    it('should insert a new world when no worldId is provided', async () => {
      mockPool.query.mockResolvedValueOnce({ rows: [{ world_id: MOCK_WORLD_ID }] });
      mockFetchWorldById.mockResolvedValueOnce(mockWorldResponse);

      const result = await upsertWorld(MOCK_USER_ID, { title: 'New World' });

      expect(result).toEqual(mockWorldResponse);
      expect(mockPool.query).toHaveBeenCalledWith(
        'INSERT INTO worlds (user_id, title) VALUES ($1, $2) RETURNING world_id',
        [MOCK_USER_ID, 'New World'],
      );
      expect(mockFetchWorldById).toHaveBeenCalledWith(MOCK_WORLD_ID);
    });
  });
  ...
  describe('updating an existing world', () => {
    it('should update a world when worldId is provided and exists', async () => {
      ...
      mockPool.query.mockResolvedValueOnce({ rows: [{ world_id: MOCK_WORLD_ID }] }); // Check exists
      mockFetchWorldById.mockResolvedValueOnce(updatedResponse);

      const result = await upsertWorld(MOCK_USER_ID, { worldId: MOCK_WORLD_ID, title: 'Updated World' });
    ...
    it('should throw WorldNotFoundError when worldId is provided but does not exist', async () => {
      ...
      await expect(upsertWorld(MOCK_USER_ID, { worldId: MOCK_WORLD_ID, title: 'Updated World' })).rejects.toThrow(
        WorldNotFoundError,
      );
    });
  });
