...
describe('user service: createUser', () => {
  ...
  it('should create a new user', async () => {
    (pool.query as jest.Mock).mockResolvedValueOnce({ rows: [] }).mockResolvedValueOnce(undefined);
...
describe('user service: addGenres', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
...
describe('user service: subscribe', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('should throw error if stripe paymentIntent failed', async () => {
    ...
    await expect(subscribe(mockUserId, mockSubscriptionRequest)).rejects.toThrow(
      'Payment failed with status: canceled',
    );
  });
});
