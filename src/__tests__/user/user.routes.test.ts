...
describe(
  'POST /user/subscribe',
  testAuth('/user/subscribe', 'post', mockSubscriptionRequest, () => {
    it('returns 200 and creates subscription', async () => {
      const res = await request(app)
        .post('/user/subscribe')
        .set(mockAuthHeaders())
        .send(mockSubscriptionRequest);
      expect(res.status).toBe(200);
      expect(res.body.status).toBe('ok');
    });
    ...
    it('returns 402 on a payment processing error', async () => {
      ...
      expect(mockSubscribe).toHaveBeenCalledWith(mockLoginResponse.userId, mockSubscriptionRequest);
    });
  }),
);
